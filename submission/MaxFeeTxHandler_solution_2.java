...
public class MaxFeeTxHandler {
	...
	public boolean isValidTx(Transaction tx) {
		// IMPLEMENT THIS
		return areCoinsExistedInPool(tx) && areSignaturesValid(tx) && isUTXOAlreadyClaimed(tx) && areAllOutputPositive(tx)
				&& checkOutputExceedsInput(tx);

	}

	// same as check output exceeds input, but returns difference instead
	private double getTxFee(Transaction tx) {
		...
		for (int i = 0; i < tx.numInputs(); i++) {
			Transaction.Input in = tx.getInput(i);
			UTXO prevUTXO = new UTXO(in.prevTxHash, in.outputIndex);
			// added to make sure tx is valid
			if (!isValidTx(tx)) {
				continue;
			}
			Transaction.Output prevTxOp = my_ledger.getTxOutput(prevUTXO);
	...
	public Transaction[] handleTxs(Transaction[] possibleTxs) {
		...
		double[] txsFees = new double[possibleTxs.length];
		// get fees from possible txs
		for (int i = 0; i < possibleTxs.length; ++i) {
			txsFees[i] = getTxFee(possibleTxs[i]);
		}
		// sort txsFees while sorting sortedTxs at the same time
		Transaction[] sortedTxs = possibleTxs;
		Transaction tempTx;
		double temp;
		for (int i = 0; i < txsFees.length; ++i) {
			for (int j = i + 1; j < txsFees.length; ++j) {
				if (txsFees[i] < txsFees[j]) {
					temp = txsFees[i];
					txsFees[i] = txsFees[j];
					txsFees[j] = temp;
					tempTx = sortedTxs[i];
					sortedTxs[i] = sortedTxs[j];
					sortedTxs[j] = tempTx;
				}
			}
		}
