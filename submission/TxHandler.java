...
public class TxHandler {
	...
	// rule (1) all outputs claimed by tx are in the current UTXO pool
	// in order to spend the coins, we need to check if coins actually exist in the
	// pool
	// the outputs claimed by tx (input of current transation) are
	// the inputs of the previous transaction
	...
	// rule (2) the signatures on each input of tx are valid
	// to validate the signature, we need to get both raw message (data) and
	// signature from
	// a transaction's input and the public key from previous transaction's output.
	// Then,
	// we use verifySignature() for signature validation
	...
	// rule (3) no UTXO is claimed multiple times by tx
	private boolean isUTXOAlreadyClaimed(Transaction tx) {
		// use HashSet to store unique UTXO or avoid duplicated UXTO
	...
	// rule (4) all of transaction's output values are non-negative
	...
	// (5) the sum of transaction's input values is greater than or equal to the sum
	// of
